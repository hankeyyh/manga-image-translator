    ...
    async def _run_text_rendering(self, config: Config, ctx: Context):
        ...
        # manga2eng currently only supports horizontal left to right rendering
            ...
            if config.render.renderer == Renderer.manga2EngPillow:
                output = await dispatch_eng_render_pillow(ctx.img_inpainted, ctx.img_rgb, ctx.text_regions, self.font_path, config.render.font_name, config.render.line_spacing, verbose=self.verbose)
            else:
                output = await dispatch_eng_render(ctx.img_inpainted, ctx.img_rgb, ctx.text_regions, self.font_path, config.render.font_name, config.render.line_spacing, verbose=self.verbose)
        else:
            output = await dispatch_rendering(ctx.img_inpainted, ctx.text_regions, self.font_path, config.render.font_name, config.render.font_size,
                                              ...
                                              config.render.disable_font_border, config.render.fit_to_box, verbose=self.verbose)
        return output
